...
const FURNITURE_OPTIONS = [
  'Диван', 'Кровать', 'Обеденный стол', 'Рабочий стол',
  'Шкаф', 'ТВ-зона', 'Кресло', 'Комод', 'Стеллаж', 'Пуф',
  'Барный стол', 'Кухонный гарнитур', 'Ванна', 'Душевая кабина',
]

const LIGHTING_OPTIONS = [
  'Естественный свет', 'Тёплый свет', 'Холодный свет',
  'Точечные светильники', 'Люстра', 'Торшер', 'Подсветка',
  'Светодиодная лента', 'Бра', 'Пендант',
]

const APPLIANCE_OPTIONS = [
  'Холодильник', 'Микроволновка', 'Посудомоечная машина',
  'Стиральная машина', 'Кухонная плита', 'Духовой шкаф',
  'Вытяжка', 'Кофемашина', 'Кондиционер', 'Телевизор',
]
...
const FLOOR_MATERIALS = [
  'Паркет светлый', 'Паркет тёмный', 'Ламинат', 'Плитка',
  'Бетон', 'Ковёр', 'Мрамор', 'Керамогранит', 'Линолеум',
]

const WALL_FINISHES = [
  'Покраска / колеровка', 'Обои', 'Декоративная штукатурка',
  'Кирпич', 'Дерево / вагонка', 'Керамогранит', 'Мрамор',
  'Гипсовые панели', 'Жидкие обои', 'Микроцемент',
  'Металлические панели', 'Стеклянные панели', 'Мозаика',
  'Имитация бетона', 'Натуральный камень', 'Пробка',
]

const TILE_ZONES = [
  'Фартук кухни', 'Пол кухни', 'Стены ванной',
  'Пол ванной', 'Стены туалета', 'Пол туалета',
  'Душевая зона', 'Вокруг ванны',
]
              ...
              <div className="details-body">
                ...
                <div>
                  ...
                  <div className="detail-chips" style={{ marginTop: 6 }}>
                    {WALL_FINISHES.map(f => (
                      <button key={f} className={`dchip${wallFinish.includes(f) ? ' on' : ''}`}
                        onClick={() => toggleArr(wallFinish, setWallFinish, f)}>{f}</button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="detail-chips" style={{ marginTop: 6 }}>
                    {FLOOR_MATERIALS.map(f => (
                      <button key={f} className={`dchip${floorMaterial === f ? ' on' : ''}`}
                        onClick={() => setFloorMaterial(floorMaterial === f ? '' : f)}>{f}</button>
                    ))}
                  </div>
                </div>
                ...
                {showTileZone && (
                  <div>
                    ...
                    <div className="detail-chips" style={{ marginTop: 6 }}>
                      {TILE_ZONES.map(z => (
                        <button key={z} className={`dchip${tilezone.includes(z) ? ' on' : ''}`}
                          onClick={() => toggleArr(tilezone, setTilezone, z)}>{z}</button>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="detail-chips" style={{ marginTop: 6 }}>
                    {FURNITURE_OPTIONS.map(f => (
                      <button key={f} className={`dchip${furniture.includes(f) ? ' on' : ''}`}
                        onClick={() => toggleArr(furniture, setFurniture, f)}>{f}</button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="detail-chips" style={{ marginTop: 6 }}>
                    {LIGHTING_OPTIONS.map(l => (
                      <button key={l} className={`dchip${lighting.includes(l) ? ' on' : ''}`}
                        onClick={() => toggleArr(lighting, setLighting, l)}>{l}</button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="detail-chips" style={{ marginTop: 6 }}>
                    {APPLIANCE_OPTIONS.map(a => (
                      <button key={a} className={`dchip${appliances.includes(a) ? ' on' : ''}`}
                        onClick={() => toggleArr(appliances, setAppliances, a)}>{a}</button>
                    ))}
                  </div>
                </div>
