...
## 8. Evolution through Composition (AI Core Evolution Axiom)

Правило (Principle 22 ADR-000): future evolution should extend
`DesignDomain` through metadata and composition before introducing new
top-level properties. Breaking changes to the `DesignDomain` contract
should be considered only when neither metadata nor composition can
express the new capability.
...
Полное обоснование, единственный официальный Decision Flow, связь с
Principle 3/19/20/21, и текст Principle 22 зафиксированы в
[ADR-000, "Update — DS-7.1.1a"](../../../../docs/adr/ADR-000-Architecture-Principles.md)
(консолидирует и заменяет две слегка разные версии, впервые описанные в
"Update — DS-7.1.1").

## 9. Decision Flow (единственный официальный, DS-7.1.1a)

Это единственный Decision Flow во всей документации AI Core — Design
Domain не имеет собственной альтернативной версии:
...
DS-7.1), поэтому шаг "Can existing metadata express it?" для него
применим напрямую. Полное обоснование (Metadata vs Composition, модели
без `metadata`, примеры, AI Core Evolution Axiom, Architect Decision
Tree, Principle Dependency Graph) — в
[ADR-000, "Update — DS-7.1.1a"](../../../../docs/adr/ADR-000-Architecture-Principles.md).

## 10. General AI Core Rule (не только Design Domain)

Тот же Decision Flow (§9) — общая философия развития **любой** модели AI
Core, а не только Design Domain: `Reuse → Metadata (если модель уже её
поддерживает) → Composition → Registry → Top-level Contract`. Шаг
`Metadata` применяется только к моделям, у которых уже есть поле
`metadata` (Design Domain — да, с DS-7.1; `PromptContext`,
`KnowledgeFeature`, `KnowledgeRelation`, Style Registry — нет, и для них
поток идёт сразу `Reuse → Composition`). Ни одна модель, ни один раздел
документации AI Core не описывает другой порядок — см. ADR-000
"Update — DS-7.1.1a" §1–§4 за консолидированной версией, таблицей
примеров и полным разбором исключения "модели без metadata".
...
Документационный этап консолидации, без единой строки runtime-кода.
`types.ts`, `domains.ts`, `registry.ts`, `index.ts` не изменены.

DS-7.1.1 (§9–§10 выше) описывал Decision Flow двумя слегка разными
версиями — специфичной для Design Domain (`Metadata` перед
`Composition`) и "общей" (`Composition` перед `Metadata`), с абзацем,
объясняющим разницу. DS-7.1.1a убирает эту неоднозначность: §9 и §10
переписаны так, чтобы во всей документации AI Core существовал ровно
один официальный Decision Flow (`Reuse → Metadata (если поддерживается)
→ Composition → Registry → Top-level Contract`) — с одним и тем же
порядком шагов везде, где он упоминается. Полная консолидация,
AI Core Evolution Axiom, Architect Decision Tree, Principle Dependency
Graph, Architecture Maturity и Universal AI Core Rule зафиксированы в
[ADR-000, "Update — DS-7.1.1a"](../../../../docs/adr/ADR-000-Architecture-Principles.md).

Design Domain по-прежнему нигде не используется, по-прежнему не
импортирует Prompt Engine, Prompt Domain, Knowledge, Knowledge Core,
Style Registry, Space Type, Developer Studio, Benchmark, API или
Production, и верхний контракт `DesignDomain` по-прежнему не изменился.
