# Candidate A Evaluation Threshold and Acceptance Plan — Revision 15 (Proposed Successor to Revision 13 — Five-Room Evaluation-Methodology Alignment; Correction-Only Successor to Revision 14)
...
Relationship to Revision 13: Revision 13 remains the sole
    ...
    accepts this Revision 15. Revision 13 is not edited, moved or
    deleted by this document.
Relationship to Revision 14: Revision 14 (Draft — Awaiting Project
    Owner Review) received one full consolidated review, verdict
    REVISE — CORRECTIONS REQUIRED, no blocking findings. Revision 14
    is not accepted, is not authoritative, and is not edited in place
    by this document. This Revision 15 is the correction-only,
    standalone successor produced from that review, superseding
    Revision 14 as the active proposal to succeed Revision 13.
    Revision 14 is retained on disk, unmodified, for historical review
    traceability (Section 21).
Trigger: Candidate A Bounded Scope Decision Revision 5 (Accepted,
    ...
    step 6). Revision 14 was prepared as that alignment decision. This
    Revision 15 is that same alignment decision, carried forward
    unchanged in substance and corrected only per the consolidated
    review of Revision 14 (Section "Revision 15 Change Summary"
    below).
...
## Revision 15 Change Summary

This section is Revision 15's own change summary, distinct from and in addition to Section 1 below (which is retained unmodified as the historical record of the Revision 13 -> Revision 14 change). Revision 15 changes from Revision 14:

```text
C-01: Synchronized repository-persistence language in Sections 19 and
    21 with the already-correct header/footer and actual review-access
    persistence state.

C-02: Corrected the self-referential cross-reference in Owner Decision
    19.

C-03: Added atomic Owner Decision 23 confirming the recommended
    ordering between the combined Preparation Plan successor and the
    Module Applicability Profile successor.

C-04: Updated decision count, traceability and completeness references
    required by Decision 23.

No evaluation arithmetic, corpus totals, Metric Registry IDs,
thresholds, support-floor dispositions, contract impacts or
architectural boundaries were changed.
```

This correction set is the complete and exclusive basis for every difference between Revision 14 and this Revision 15.

Revision 15 is a standalone correction-only successor to Revision 14. It preserves Revision 14's substantive evaluation architecture, arithmetic, Metric Registry, thresholds, support-floor dispositions, contract impacts and non-authorization boundaries, except for the explicit correction set recorded in this change summary (C-01 through C-04).

All current-document self-references have been normalized to Revision 15. References to Revision 14 are retained only where they describe the historical reviewed draft, its repository persistence, its review findings, or the delta from Revision 14 to Revision 15. Section 1 below is retained unmodified as the historical record of the Revision 13 -> Revision 14 change (see Section 1's own heading and text), and is not itself a current-document self-reference.
...
6. Establish the exact non-authorization boundary (Section 19) and governance sequence (Section 17) so that accepting this Revision 15 activates toilet room for evaluation-methodology purposes only — never for corpus creation, annotation, provider/model evaluation, or implementation.
...
4. docs/engineering-decisions/reviews/
   ...
   Revision 15's own directory listing, not to be present on disk.)
...
9. docs/engineering-decisions/reviews/
   ...
   historical four-room baseline this Revision 15 supersedes at the
   evaluation-methodology layer.)
```

*Non-normative editorial note (this Revision 15 only): line counts cited above and elsewhere in this document may differ by one across tools depending on whether the final non-terminated line or trailing newline is counted. Document identity is established by path, revision, content and repository commit, not by line count alone. This note does not alter, and is not, an acceptance criterion.*

**Local drafts confirmed absent from the repository, and therefore not usable as sources:** `Candidate-A-Bounded-Scope-Decision-Rev4.md` and `Candidate-A-Contracts-1-10-Preparation-and-Dependency-Plan-Rev5.md`. Both are referenced inside Bounded Scope Rev5's own text (§1, §11.3) as historical or cross-session artifacts, but neither file exists on disk in this repository as of this document's preparation date, confirmed by this Revision 15's own directory listing performed independently of Rev5's claim.
...
Bounded Scope Rev5 §9.1 names seven directly room-count-keyed Rev13 provisions (§2; §10.1; §10.2 corpus totals; §6.3; §16; §18; §19). This Revision 15's own full re-reading of Rev13 (baseline 1 above) identifies one additional directly room-count-keyed provision not named there:
...
This Revision 15 imports this list by reference. It does not redefine, expand, or reinterpret it. Rev13 §2's bullet — *"Room types: living room, bedroom, kitchen and bathroom"* — is replaced in this successor by:
...
Upon Project Owner acceptance of this Revision 15, and only then, the evaluation status changes from *Pending* to *Active for evaluation-methodology purposes*. This is a narrower activation than full operational readiness: it means toilet room's grid cells, denominators, and applicable metric rows are fixed and internally consistent (Sections 8-10 below) and toilet room may be referenced normatively in future Contracts 1-10 and Contract 11 drafting. It does **not** mean corpus preparation, annotation, provider/model evaluation, or implementation involving toilet room may begin — those remain gated by the unchanged Rev13 §5.2 governance route and the explicit non-authorization boundary (Section 19).

```text
toilet room, upon acceptance of this Revision 15:
  Bounded scope:                 accepted (Rev5, unchanged)
  Evaluation-methodology status: active (this Revision 15)
...
The accepted segment remains `Residential-first`. This Revision 15 does not expand, narrow, or reinterpret it, and does not activate any other segment. This mirrors Rev5 Decision 4 and is restated here as an explicit boundary of this document (Section 18, Decision 3).

---

## 5. Bathroom / Toilet-Room Evaluation Distinction

Bounded Scope Rev5 §7 defines the normative distinction between `bathroom` and `toilet room`. This Revision 15 imports it by reference in full and does not redefine it:
...
| Visually ambiguous sanitary room (no bathtub/shower visible, toilet visible, unclear if a tub/shower exists outside frame) | Governed by Rev5 §7.4 rule 2 on visible evidence only — `toilet room` if no bathtub/shower is visible in-frame | This is a visible-evidence classification, not a "completeness: partial" case per se; Contract 7 must state explicitly that classification precedence uses only in-frame visible evidence, consistent with Rev13 §10.4's sufficiency/completeness model, which this Revision 15 does not alter. |
| Cropped image where the bath/shower is outside the frame | Ambiguity is possible; Rev5 does not resolve this case explicitly | Flagged as an open ambiguity-handling item for Contract 7 (Section 7 below), not resolved by this Revision 15. |
...
### 6.1 What this Revision 15 does fix: the evaluation-level consequence, conditional on future applicability

If, and only if, a future Contract 3 makes any relation type applicable to toilet room, the following Rev13 provisions already apply automatically by their existing generic wording, and are not re-derived or changed by this Revision 15:
...
- "How is a partial image handled?" Rev13 §10.4's existing
  ...
  no new criterion is created for toilet room by this Revision 15.
- "How is label leakage avoided?" Out of scope for Rev13 and this
  Revision 15; this is an annotation-procedure question for the
...
- When is a room considered `bathroom` vs. `toilet room` specifically
  ...
  written for the fully-visible case. This Revision 15 does not
...
**No room-type-specific exclusion applies to any of the six scenario families for toilet room, and no `not-applicable` cell is required.** This finding is fixed normatively by this Revision 15 (Section 18, Decision 4), consistent with the instruction to fix it only if the full check supports it — which it does here — rather than defaulting to symmetry unexamined.
...
New (five-room, this Revision 15):
  5 room types x 6 scenario families = 30 cells
  development: 30 x 2/cell = 60
  held-out:    30 x 3/cell = 90
```

Per-cell minimums (development 2, held-out 3) are **unchanged** — this Revision 15 does not alter the per-cell minimum, only the number of cells.
...
No. Bounded Scope Rev5 §9.1 already established, and this Revision 15 independently reconfirms by re-reading Rev13 §10.2 and Rev3 Part I.2's "обязательные cross-room scenario groups" framing (cited in Rev5 §9.1), that these four groups are **pooled cross-room, not partitioned per room type**. They are therefore not directly room-count-keyed and require **no numeric change** merely because a fifth room type is accepted.
...
| **Overall evaluation population** (semantic corpus + all fixture suites, held-out side) | 103 + 29 + 18 = 150 | 121 + 29 + 18 = 168 | Sum of held-out semantic total and held-out fixture totals; fixture totals unaffected | §10-§12 (this row is this Revision 15's own reconciliation, not a Rev13-stated aggregate — Rev13 does not itself sum semantic and fixture populations into one figure, and this Revision 15 does not introduce that as a new normative total, only as an explanatory reconciliation) |
...
This rule is already written generically per room type. It applies automatically to toilet room once (and only once) a category floor is actually reported for it — which requires the corpus/annotation work this Revision 15 does not authorize. No re-derivation is needed; this is confirmed unchanged.
...
All 81 Rev13 §19 Metric IDs are accounted for below by category. No metric ID is renamed, added, or removed by this Revision 15 — a requirement explicit in the drafting instructions ("не меняй metric IDs без необходимости"). Category assignment is exhaustive and disjoint; totals are cross-checked against Rev13's own stated count of 81.
...
Contract 11 (Aggregation, Uncertainty and Score-Stability Appendix) is not prepared, drafted, or prejudged by this Revision 15, consistent with Rev13 §5.2 step 6 (Contract 11 is prepared *after* development corpus/fixture creation, using actual development denominators) and the Contracts 1-10 Preparation and Dependency Plan Rev4 §7 boundary against contract 11.

This Revision 15 fixes:
...
3. No premature aggregation decision is made here. This Revision 15
...
None of Contracts 1-10 has been drafted (confirmed by Module Applicability Profile Rev13, matrix row CON-01: `Not Started`). This matrix addresses the **future scope and acceptance boundary** each contract will need to honor once drafted, updated for the evaluation-methodology alignment this Revision 15 performs — distinct from, and in addition to, Bounded Scope Rev5 §10's scope-layer impact analysis.

| Contract | This Revision 15's evaluation-methodology impact |
|---|---|
...
| **Contract 3** — Relation Type x Room Type Applicability Matrix | Must target the five-room model (Bounded Scope Rev5 §10.1 recommendation, reaffirmed here). Toilet-room rows may be held Deferred until Contract 3's own drafting resolves relation applicability (Section 6) — this Revision 15 does not decide that applicability, only that the matrix's target shape is five rooms, not four. |
| **Contract 4** — Best-Effort Evidence, Provenance and Determinability Annotation Contract | Room-type independent. Not affected by this Revision 15. |
| **Contract 5** — Confidence Generation and Normalization Contract | Room-type independent. Not affected. |
| **Contract 6** — Unknown/Determinability Annotation and Pairing Contract | Annotation-unit pairing is keyed to entity subtype/confidence/provenance fields, not room type directly. Must support toilet-room cases once they exist, but pairing semantics themselves are not redefined by this Revision 15. |
...
| **Contract 8** — Unseen-Claim Evaluation Artifact Contract | Depends on Contract 7's five-room case population; toilet-room unseen-claim cases become possible once Contract 7 exists, but this Revision 15 does not create or authorize them. No independent room-type dependency beyond that inheritance. |
| **Contract 9** — Operational and Contract Violation Fixture Subtype Registry | Confirmed unaffected — fixture subtypes are operational/technical and room-count independent (Section 10.1). No toilet-specific failure fixtures are created by this Revision 15; whether any are needed is Contract 9's own future determination, not decided here. |
| **Contract 10** — Conformance Field Inventory and Validation Contract | Five-room conformance profile follows automatically once Contracts 1, 3, 7 are updated for five rooms (integration layer, Preparation Plan Rev4 §2). Toilet room is considered active for Contract 10's purposes only after this Revision 15 is accepted **and** the later Contract 1 lock — consistent with the Preparation Plan's existing candidate-lock/finalization model, unchanged by this Revision 15. |

No drafting, revision, or acceptance of any Contract 1-10 is authorized by this Section or by this Revision 15 as a whole (Section 19).

---

## 16. Category Vocabulary Dependencies

This Revision 15 does not draft Contract 1. It names the minimum evaluation-level vocabulary dependencies toilet-room evaluation will require, for the Active Evaluation Profile to be internally consistent — and explicitly defers the full vocabulary contract to the future Contract 1 and any future Preparation Plan successor.
...
**No new top-level node class is introduced by this Revision 15.** Every candidate item above is a subtype-level or field-level consideration for the future Contract 1, not a schema-level change. This table does not write Contract 1; it names dependencies Contract 1 must resolve and explicitly defers the full vocabulary decision.

---

## 17. Governance Sequence

The following sequence is this Revision 15's own proposal, checked against the authoritative dependency structure of the imported baselines (Section 3) rather than asserted unchecked.

```text
1. Draft Evaluation Threshold and Acceptance Plan Revision 15 (this
   document).
...
4. Separate, explicit repository persistence authorization for
   Revision 15 (not granted by acceptance alone; not granted here).
5. Upon (3), Revision 15 supersedes Revision 13 in full as the
...
6. A combined Preparation Plan successor revision, reconciling (per
   Bounded Scope Rev5 §11.3, unresolved by this Revision 15 and
...
8. Separate, explicit Project Owner authorization to begin drafting
   ...
   Rev13 §17 item 9; this Revision 15 does not change that gating and
...
9. Contracts 1-9 drafted in the dependency order already fixed by the
   ...
   (Bounded Scope Rev5 §10.1, reaffirmed by this Revision 15 Section
   15).
...
The drafting instructions ask this Revision 15 to specifically evaluate whether the Module Applicability Profile successor synchronization should precede or follow the combined Preparation Plan successor. This Revision 15's assessment:
...
This Revision 15 does not itself perform either synchronization pass (Section 19).

**No document beyond this Revision 15 is started automatically by this document's acceptance.** Steps 6-10 above are a proposed sequence, not authorizations.
...
```text
Decision 1 — Acceptance of Revision 15: Accept / Revise / Reject this
    Revision 15 as the successor Evaluation Threshold and Acceptance
...
Decision 3 — Operational activation of toilet room for evaluation
    methodology: Confirm that, upon acceptance of this Revision 15,
...
Decision 12 — Relation applicability treatment: Confirm that Adjacency,
    ...
    decided by this Revision 15 and remains reserved to the future
...
Decision 17 — Contract 11 dependency: Confirm that Contract 11 is not
    prepared, drafted, or prejudged by this Revision 15 (Section 14),
...
Decision 19 — Module Applicability Profile synchronization
    ...
    types" language in the Module Applicability Profile's own §2,
    IMPL-04 and EVAL-01. See this document's Section 17.1 (governance-
    sequence ordering assessment), Section 20 traceability matrix, and
    Section 22 Risk R-3 for the supporting analysis. References to the
    Module Applicability Profile's own Applicability Matrix refer to
    that separate authoritative document, not to Section 18 of this
    Plan (this Owner Decision list). Also acknowledge the additional
    Bounded-Scope-Rev3-citation staleness found in BASE-00A, BASE-01,
    GOV-03, PROV-01 and PROV-05 as a lower-priority, separate
...
Decision 22 — Non-authorization of execution: Confirm that this
    Revision 15, like Revision 13 before it, authorizes none of:
...
Decision 23 — Sequencing between the combined Preparation Plan
    successor and the Module Applicability Profile successor: Confirm
    the following recommended sequence, independent of and additional
    to Decisions 19 and 20 (which each establish that a successor
    document is separately required at all, not the order between
    them):
    ...
    2. Prepare and accept the combined Preparation Plan successor
       revision (Section 17 step 6; Decision 20).
    3. Prepare and accept the Module Applicability Profile successor
       synchronization pass (Section 17 step 7; Decision 19).
    4. Begin Contract 1 drafting (Section 17 step 8, subject to its
       own separate, not-yet-granted Project Owner authorization).

    Rationale: the Preparation Plan successor determines the actual
    dependencies, boundaries and drafting order of Contracts 1-10; the
    Module Applicability Profile successor then synchronizes the
    Profile's applicability-state with the already-agreed five-room
    evaluation architecture and the updated Plan. Both successor
    documents must be closed before Contract 1 drafting begins. The
    Project Owner may reorder this sequence by a separate future
    decision, without contradicting this Decision 23. This Decision 23
    does not authorize drafting, revising, or accepting either
    successor document, and does not duplicate Decision 19 (necessity
    of Profile synchronization) or Decision 20 (necessity of the
    Preparation Plan successor) — it fixes only the order between the
    two.
```

Methodology acceptance (Decisions 1-4, 10-18), corpus authorization, implementation authorization, repository persistence (Decision 22), and future successor-document sequencing (Decision 23) are kept as distinct, non-overlapping decisions, per the explicit drafting instruction not to mix these categories. This Plan now carries 23 Owner Decision entries, numbered continuously 1-23.

---

## 19. Explicit Non-Authorization Boundary

This Revision 15 does not authorize:

```text
- Modification, deletion, or repository change of Revision 13.
- Already completed, and not reopened or reauthorized by this
  document: review-access repository persistence of the Revision 14
  draft, in commits bd76df3f2da01cdcd572bd72bc53c60b49773a90 (added
  the Revision 14 draft for review) and
  5ca8c39fdd57aacf4200567e0607761f891961dc (synchronized Revision 14's
  persistence metadata). This is recorded here as historical fact
  only; neither commit constitutes, and this document does not
  represent either commit as, Project Owner acceptance of Revision 14.
- Not authorized by this document: repository persistence of this
  Revision 15 itself; creation of an Owner Acceptance record for
  Revision 14 or for this Revision 15; supersession metadata recording
  this Revision 15 (or Revision 14) as superseding Revision 13;
  persistence of any downstream document; and any other execution
  authorization.
...
| Revision 15 provision | Source basis | Status |
|---|---|---|
...
| Toilet room two-stage status and this document's role as the Rev13-alignment successor | Bounded Scope Rev5 §6.1, §9.3, §11.1, §11.2 step 6 | Direct trigger for this Revision 15 |
| Bathroom/toilet-room normative distinction (Section 5) | Bounded Scope Rev5 §7 | Imported unchanged; evaluation-dataset case table is this Revision 15's own addition |
...
| 5x6 ordinary grid, 30 cells, 60/90 images (Section 8) | Rev13 §10.1, read in full (Section 3) | Recomputed by this Revision 15 |
...
| Corpus totals 75 development / 121 held-out (Section 10.1) | Derived from Sections 8-9 | New arithmetic, this Revision 15 |
| §15.1 "four room types" -> "five room types" correction (Section 10.3) | Rev13 §15.1, identified by this Revision 15's own full reading (Section 3.1), not named in Bounded Scope Rev5 §9.1 | New finding, this Revision 15 |
...
| Relation GT support floor non-formulaic proof (Section 11.2) | Rev13 §16, arithmetic check performed by this Revision 15 | New analysis, this Revision 15 |
| Ten-row Blocking support-floor Open disposition (Section 12) | Rev13 §18/§19, row-by-row check performed by this Revision 15 | New analysis, this Revision 15 |
| 81-row Metric Registry categorization (Section 13) | Rev13 §19, read in full and categorized by this Revision 15 | New categorization, cross-checked to Rev13's stated total |
| Contract 1-10 impact matrix (Section 15) | Preparation Plan Rev4 §2; Bounded Scope Rev5 §10 (scope-layer precedent) | Extended to the evaluation-methodology layer by this Revision 15 |
| Category vocabulary dependency inventory (Section 16) | Bounded Scope Rev5 §9.1 R-5; drafting-instruction candidate list | New inventory, this Revision 15; explicitly non-binding |
...
| Module Applicability Profile IMPL-04/EVAL-01/§2 and BASE-00A/BASE-01/GOV-03/PROV-01/PROV-05 citation staleness | Module Applicability Profile Rev13, read in full (Section 3) | Identified as affected; not modified (this Revision 15 does not edit the Profile) |
| Sequencing between the combined Preparation Plan successor and the Module Applicability Profile successor (Owner Decision 23) | Section 17.1's non-binding ordering assessment, formalized as an Owner Decision by this Revision 15 | New Owner Decision, this Revision 15 (Section "Revision 15 Change Summary," C-03) |
...
```text
This Revision 15 does not supersede Revision 13 upon preparation,
...
until the Project Owner explicitly accepts this Revision 15.

Upon explicit Project Owner Acceptance of this Revision 15 (Section
18, Decision 1), and only then, this Revision 15 supersedes Revision
13 in full. No partial or conditional supersession is defined: either
Revision 15 is accepted and fully replaces Revision 13, or it is not
accepted and Revision 13 continues to govern unchanged.

Repository persistence of a revision of this Plan (i.e., committing a
working draft so that it is no longer merely local) is a separate,
explicit authorization, distinct from acceptance itself, exactly as
was true for Revision 13, for Bounded Scope Decision Revision 5, and
for Revision 14.

Review-access persistence has occurred. Project Owner acceptance has
not occurred. Revision 13 remains authoritative.

Specifically: review-access repository persistence of the Revision 14
draft has occurred, in commits
bd76df3f2da01cdcd572bd72bc53c60b49773a90 (added the Revision 14 draft
for review) and 5ca8c39fdd57aacf4200567e0607761f891961dc (synchronized
Revision 14's persistence metadata). Project Owner acceptance of
Revision 14 has not occurred. Neither commit is represented by this
document as acceptance.

Repository persistence of this Revision 15 has now been authorized and
completed, for verification access only, distinct from, and in
addition to, the authorization already given for Revision 14's
review-access persistence. This verification-access persistence does
not constitute Project Owner acceptance of this Revision 15. Revision
13 remains authoritative until separate acceptance of this Revision
15.
```
...
R-4 (open ambiguity-handling item, not resolved by design): The
    ...
    is not resolved by Bounded Scope Rev5 §7.4 or by this Revision 15.
    It is reserved to the future Contract 7. This Revision 15 does not
...
R-5 (unaddressed by design, inherited from Bounded Scope Rev5 R-5):
    ...
    very-low-confidence `urinal` candidate, which this Revision 15
...
R-6 (governance-sequence judgment call, not a defect): The relative
    ...
    successor (Section 17.1) was originally this Revision 14's own
    non-binding recommendation, not a dependency-graph fact discovered
    in the source documents — the sources are silent on this specific
    ordering question. This Revision 15 formalizes that recommendation
    as Owner Decision 23 (Section 18), while preserving the Project
    Owner's ability to reorder these two future steps by a separate
    future decision without contradicting anything in this Revision
    15.
```
...
[x] One additional room-count-keyed Rev13 provision (§15.1) found by
    this Revision 15's own full reading, beyond Bounded Scope Rev5's
...
[x] Bathroom/toilet-room distinction imported by reference; evaluation-
    dataset case table added as this Revision 15's own contribution,
...
[x] (This Revision 15 only) Owner Decision 23 added; 23 Owner Decision
    entries exist, numbered continuously 1-23; Decisions 19, 20 and 23
    are non-duplicative (Section 18); the Decision 19 cross-reference
    resolves to an existing section, not a self-reference (Section
    18); Sections 19 and 21 agree with the header and footer on
    repository-persistence status (Section "Revision 15 Change
    Summary," C-01 through C-04).
```
...
Revision 15. Not Accepted. Persisted for historical review traceability
only, in commit bd76df3f2da01cdcd572bd72bc53c60b49773a90 (draft added for
review) and commit 5ca8c39fdd57aacf4200567e0607761f891961dc (persistence
metadata synchronized). Revision 14 is not authoritative.

Revision:
15 (proposed successor to Evaluation Threshold and Acceptance Plan
Revision 13; correction-only successor to Revision 14).

Toilet room:
Bounded scope: accepted (Bounded Scope Decision Rev5, unchanged by
this document).
Evaluation-methodology status upon acceptance of this Revision 15:
...
Revision 14: Not modified, not deleted, not moved.

Schema, corpus, provider, and implementation authorization:
Not granted by this document.

Repository changes performed during preparation of this Revision 15:
NO.

Repository persistence of this Revision 15:
YES. Authorized and completed for verification access only, distinct
from Revision 14's own review-access persistence (commits
bd76df3f2da01cdcd572bd72bc53c60b49773a90 and
5ca8c39fdd57aacf4200567e0607761f891961dc), which does not extend to this
Revision 15. This verification-access persistence does not constitute
Project Owner acceptance of Revision 14 or of this Revision 15.

File state (this Revision 15):
Tracked. Committed. Pushed to origin/main.
```
