# Candidate A Test Data Handling Decision — Revision 10 (Residential-34 / RoomCase-ImageAsset Identity Alignment Successor to Revision 9)

**Document type:** Proposed Owner Governance Decision (not an ADR; not an Implementation Package) — standalone successor candidate  
**Revision type:** Residential-34 scope synchronization and multi-image (`RoomCase`/`ImageAsset[1..6]`) identity-model alignment  
**Status:** Draft — Awaiting Project Owner Review  
**Revision:** 10  
**Supersedes:** Revision 9 (Accepted, Project Owner, 2026-07-16) — not modified, not reopened, remains the sole authoritative Test Data Handling Decision until this Revision 10 is separately accepted  
...
**Supporting-contract preparation:** Not authorized (Contract 1 Revision 19 and Contract 2 Revision 10 are imported as already-completed, candidate-locked inputs; neither is redrafted or reopened here)  
...
## Revision 10 Change Summary

This is a complete, standalone successor candidate to Revision 9. Sections 4-19 and 21-23 were checked in full for room-count, room-type, and image-count dependencies and retain their substantive Revision 9 rules. Section 20 carries the explicitly disclosed sequencing synchronization in C-09. Section 24 carries only the explicitly disclosed acceptance, non-authorization, sequence, current-baseline, identity, and Revision 10 Owner-decision changes in C-10-C-13. Sections 25-26 and the footer carry only the status and integrity-accounting changes in C-14. Every Revision 10 difference is recorded below.

```text
Changed (Revision 9 -> Revision 10):

C-01 (§2, item 2). "One perspective room image, one room, one
    operation" is replaced by the Operation / RoomCase / ImageAsset[1-6]
    model (Bounded Scope Rev5 §8A). See Section 2 below.

C-02 (§2, item 4). The blanket exclusion of "multi-image input,
    multi-view fusion, ... cross-image reasoning" directly contradicted
    the accepted Bounded Scope Rev5 §8A-8B model, which requires
    same-RoomCase multi-image fusion (1-6 ImageAsset per RoomCase,
    cross-view matching, deduplication, contradiction preservation,
    evidence fusion) as an in-scope capability. Corrected to exclude
    only cross-session/persistent multi-view and whole-home reasoning,
    which remain out of scope. See Section 2 below.

C-03 (§2, item 5). "Tier 1 room types: living room, bedroom, kitchen
    and bathroom" is replaced by a reference to the full 34-category
    Residential-34 list (Bounded Scope Rev5 §6), not restated in full
    here to avoid forking Contract 1's ownership of the vocabulary.
    See Section 2 below.

C-04 (new §3.3.0). A new identity-model section distinguishes
    PerceptionOperation, SourceGenerationOperation, and
    MixedRoomValidationFixtureOperation. `roomCaseId` is mandatory for
    valid perception RoomCases, but is not fabricated for text-only
    source generation or mixed-room negative input sets. `imageAssetId`
    and atomic `sourceAssetId` remain one-per-photograph.

C-05 (§3.3.1 amended). Operation-scoped identity fields are made
    operation-type-aware. Every RoomCase-level input or output artifact
    carries `contributingImageAssetIds[]` with cardinality 1..6; per-image
    artifacts retain their own atomic `imageAssetId`/`sourceAssetId`.

C-06 (new §3.3.1A). New evidentiary categories required by the
    RoomCase/ImageAsset model are named and governed as data: same-room
    image sets, mixed-room negative sets, cross-view evidence and
    cross-view matching, duplicate-object cases, contradiction cases,
    evidence fusion records, per-image provenance, and RoomCase-level
    provenance. See Section 3.3.1A below.

C-07 (§3.3.7 closing statement). The forward cross-reference to
    "Evaluation Threshold and Acceptance Plan Rev13" — accurate when
    Revision 9 was written, now stale — is updated to cite the current
    Revision 15 / proposed Revision 16. Found during this document's
    own adjacent-field self-check (Section 26 below), not part of the
    original B1-B4 finding set; recorded here for completeness rather
    than left as a silent drive-by edit.

C-08 (header metadata). Records the Engineering Architect correction
    and consolidated-review role for this in-place Revision 10 update.

C-09 (§20). The Corpus Preparation admission gate is synchronized to
    the already-accepted Module Applicability Profile sequence. In
    addition to atomic Contracts 1-10 acceptance and the Section 22
    pre-corpus artifacts, it now explicitly requires completion of the
    mandatory Diagnosability/Security sequence and acceptance of the
    Phase-1 Scope Decision or Execution Profile before Tier 1 Corpus
    Preparation Authorization may be considered. This imports already
    accepted sequencing; it does not create or authorize those steps.

C-10 (§24, Decisions 15 and 18). Decision 15 is synchronized to the
    complete Section 20 admission gate, including the mandatory
    Diagnosability/Security and Phase-1 prerequisites. Decision 18 is
    synchronized to the actual Phase 6 state: Contract 1 Revision 19
    and Contract 2 Revision 10 are completed/candidate-locked inputs;
    Contracts 3-10 remain not authorized; one-time Project Context /
    Roadmap synchronization and the final root-package review precede
    any later Contracts 3-10 cycle.

C-11 (§24, Decisions 1, 17, and 19). Decision 1 names Revision 10 as
    the acceptance target. Decision 17 updates the non-authorization
    boundary to distinguish completed Contracts 1-2 from non-authorized
    Contracts 3-11 drafting. Decision 19 replaces stale Evaluation
    Threshold Plan Rev13 wording with the accepted Revision 15 baseline
    and its Phase 6 successor Revision 16, matching C-07's operative
    reference correction in §3.3.7.

C-12 (§24, Decision 20). The operation-type-aware identity decision is
    updated to incorporate the complete no-fabrication rule: valid
    PerceptionOperations require roomCaseId; SourceGenerationOperations
    do not fabricate it before RoomCase admission; mixed-room negative
    fixtures use inputSetId plus atomic per-image identities and the
    expected mixed_room_rejected outcome. The formerly separate,
    duplicative Decision 42 is removed and its complete substance is
    consolidated into Decision 20; no normative rule is lost.

C-13 (§24, Decisions 38-41). Four Revision 10 decisions explicitly
    present for Owner review the Residential-34 reference, the
    Operation/RoomCase/ImageAsset[1-6] model, atomic per-image identity
    and complete contributingImageAssetIds[] rule, and the nine
    multi-image evidentiary data categories. These decisions correspond
    to C-01-C-06 and do not add a second source of architecture.

C-14 (§25-§26 and footer). Current-document status, completeness
    accounting, Decision references, and final integrity statements are
    synchronized to Revision 10 and to the complete C-01-C-14 change
    inventory above.

Sections 4-19 and 21-23 retain their substantive Revision 9 rules.
Section 20 changes only as explicitly disclosed in C-09. Section 24
changes only as explicitly disclosed in C-10-C-13 and the previously
listed identity/scope items. Sections 25-26 and the footer change only
as disclosed in C-14. No corpus creation, real-user-data use,
provider/model evaluation, implementation, or production activation is
authorized.
```

This correction set is the complete and exclusive basis for every difference between Revision 9 and this Revision 10. Revision 9 is not edited, moved, or deleted by this document.
...
- alter the accepted Candidate A bounded scope or the accepted/proposed Evaluation Threshold and Acceptance Plan baseline (Revision 15 / Phase 6 successor Revision 16).

Acceptance of this document would complete **Phase 6, item B: Test Data Handling Decision synchronization**. It does not repeat or reopen the earlier governance-sequence acceptance of Revision 9.
...
2. **[Revision 10, C-01]** For every valid Perception operation: one `RoomCase`, corresponding to exactly one physical/staged room; one to six `ImageAsset` objects per `RoomCase`, all materially depicting the same room in the same state (Bounded Scope Rev5 §8A). This replaces the historical "one perspective room image, one room, one operation" model of Revisions 1-9.
3. Permitted source classes: licensed, synthetic and deliberately staged.
4. **[Revision 10, C-02]** Excluded: real user photos, personal data, panoramas, floor plans, video, persistent cross-session multi-view evidence accumulation, and whole-home image sets. Same-`RoomCase` multi-image fusion (cross-view matching, deduplication, contradiction preservation, evidence fusion across the 1-6 `ImageAsset` objects of one `RoomCase`) is **in scope** as of this Revision 10 (Bounded Scope Rev5 §8A-8B) and is no longer excluded — Revisions 1-9's blanket exclusion of "multi-image input, multi-view fusion... cross-image reasoning" is corrected to this narrower, current-model-consistent boundary.
5. **[Revision 10, C-03]** Residential-34: the complete 34-category residential room/space scope defined by Candidate A Bounded Scope Decision Revision 5 §6 (as in-place corrected) and Candidate A Supporting Contract 1 — Master Vocabulary and Active Evaluation Profile, Revision 19 §2.3 (candidate-locked, C1-REV19-CL-001). Not restated here in full, to avoid forking Contract 1's ownership of the vocabulary; this Decision references the list by citation only, preserving Contract 1's vocabulary ownership.
...
9. **[Revision 10 note]** Evaluation artifacts and supporting contracts remain governed by the Evaluation Threshold and Acceptance Plan — currently Revision 15 (Accepted, 2026-07-18), with a proposed Residential-34 successor (Revision 16) prepared alongside this document as the first item in the same Phase 6 sequence — and by later separately accepted Supporting Contracts.
...
#### 3.3.0 Operation / RoomCase / ImageAsset identity model **[Revision 10, C-04]**

The accepted bounded runtime has one valid Perception `RoomCase` per Perception operation and 1-6 `ImageAsset` objects per valid RoomCase. Test-data governance also covers source-generation operations and negative validation fixtures that are not themselves valid RoomCases. Identity requirements therefore depend on operation type:

```text
A. PerceptionOperation
   operationId
   └── roomCaseId [required; exactly one valid physical/staged room]
       └── imageAssetId[1..6] ↔ sourceAssetId[1..6]

B. SourceGenerationOperation
   operationId [or equivalent governed generation-operation identity]
   └── generation request / prompt / configuration identity
   roomCaseId: not applicable until a generated asset is admitted into
               a governed RoomCase; it must never be fabricated.

C. MixedRoomValidationFixtureOperation
   operationId
   └── inputSetId [required; 2..6 governed image inputs intentionally
                   not asserted to depict one room]
   roomCaseId: absent because the expected outcome is mixed-room
               rejection; it must never be fabricated.
```

Identity meanings:

```text
operationId   — one governed operation of the applicable type.
roomCaseId    — one valid Perception RoomCase corresponding to one
                physical/staged room; required only where such a valid
                RoomCase exists.
inputSetId    — logical identity for one governed negative-validation
                input set that is not asserted to be a valid RoomCase;
                exact field spelling remains Contract 10 responsibility.
imageAssetId  — one governed photograph admitted as an ImageAsset.
sourceAssetId — one atomic governed source asset for that photograph.
```

For every admitted photographic ImageAsset, the binding is invariant:

```text
one ImageAsset ↔ one imageAssetId ↔ one sourceAssetId
```

`sourceAssetId` is never set-valued. This preserves per-file provenance, licensing, source-class attribution, auditability, incident handling, retention, and deletion traceability.

Every RoomCase-level input or output artifact — including the assembled request payload, `StructuredSceneV0`, `PerceptionEvidenceArtifact`, evidence-fusion record, and consolidated `PerceptionResult` — carries `operationId`, `roomCaseId`, and `contributingImageAssetIds[]` with cardinality 1..6, naming every contributing ImageAsset. Per-image artifacts carry their own atomic `imageAssetId` and `sourceAssetId`.

A mixed-room negative fixture carries `operationId`, `inputSetId`, `fixtureId`, and the atomic identities of all contributing governed images, plus expected outcome `mixed_room_rejected`; it does not carry a fabricated valid `roomCaseId`.

This Decision fixes identity levels and no-fabrication rules. Exact field names, JSON/envelope serialization, and final alignment among `sourceImageId`, `inputArtifactId`, `sourceAssetId`, `imageAssetId`, `roomCaseId`, and `inputSetId` remain Contract 10 responsibilities.
...
**Operation-input identity.** Every operation-scoped artifact must carry fields appropriate to its operation type:

- `operationId` — required for every governed operation;
- `inputArtifactId` — required for every operation, including malformed, missing-image, unsupported, rejected, fixture-backed, and source-generation inputs;
- `roomCaseId` — required for valid PerceptionOperations and their RoomCase-level artifacts; prohibited from fabrication for text-only SourceGenerationOperations or mixed-room negative input sets;
- `inputSetId` — required for a mixed-room negative fixture input set that is intentionally not a valid RoomCase;
- `sourceAssetId` and `imageAssetId` — required as an atomic pair for each admitted governed photograph actually used; neither is fabricated or made set-valued;
- `contributingImageAssetIds[]` — required for every RoomCase-level input or output artifact, including the assembled request payload and consolidated result artifacts; cardinality 1..6, unique values only, and equal to the complete contributing ImageAsset set;
- `contributingInputImageAssetIds[]` (or Contract-10-equivalent field) — required for a mixed-room negative fixture request; cardinality 2..6 and tied to `inputSetId`, not to a valid `roomCaseId`;
- `fixtureId` and `fixtureLineageId` — required where fixture governance applies;
- `lineageId` — required per photographic lineage where applicable;
- `subset`, `producingStage`, `timestamp`, and `contentHash` — required where applicable under the common handling model.

Every operation must remain traceable to a real governed input, fixture, or generation-request identity. No fabricated `sourceAssetId`, `imageAssetId`, `roomCaseId`, `inputSetId`, `lineageId`, `fixtureId`, or `fixtureLineageId` may be created.

**Example — text-only synthetic generation.** A synthetic generation request with no reference image has no `sourceAssetId`; none may be fabricated. Its identity rests on the generation request/prompt identity, `operationId`, immutable `sourceGenerationConfigurationId` (Sections 16.0.1–16.0.2), and the synthetic generation lineage required by Section 4.2. No `roomCaseId` applies until a generated output is admitted into a valid RoomCase. Where a reference or source image is used, its governed `sourceAssetId` and `lineageId` must also be recorded.
...
#### 3.3.1A Multi-image RoomCase evidentiary categories **[new, Revision 10, C-06]**

Per explicit Project Owner instruction (2026-07-22), this Decision names and governs, as data, the following categories required by the `RoomCase`/`ImageAsset[1..6]` model. Each is operation/RoomCase-scoped (Section 3.3.1) unless stated otherwise, and is subject to the same source-eligibility (Section 4), privacy (Section 7), access-control (Section 9), and retention (Section 17) rules as any other governed asset or artifact:

- **same-room image sets** — the 1 to 6 `ImageAsset` objects of one `RoomCase`, all materially depicting the same physical/staged room in the same material state (Bounded Scope Rev5 §8A "capture-set invariants"); governed as a set under the shared `roomCaseId`, with each member independently eligible under Section 4;
- **mixed-room negative sets** — deliberately constructed fixture entries in which 2-6 images do *not* depict the same room, used to test mixed-room rejection; governed through `inputSetId`, `fixtureId`, and atomic per-image identities, with no valid `roomCaseId` asserted or fabricated; never conflated with an eligible same-room set;
- **cross-view evidence** — annotation/evaluation records describing what a given scene element or relation looks like across more than one `ImageAsset` within one `RoomCase`; corpus-scoped or operation-scoped depending on whether it is ground truth (corpus-scoped, Section 3.3.3) or execution output (operation-scoped, Section 3.3.1);
- **cross-view matching** — the record of which detections/claims across different `ImageAsset` objects of the same `RoomCase` are treated as referring to the same real-world entity; operation-scoped, carrying `contributingImageAssetIds[]` (Section 3.3.1);
- **duplicate-object cases** — annotation records distinguishing a genuine duplicate real-world object from the same object re-observed across views; corpus-scoped (ground truth) or operation-scoped (mechanism output), per the same rule as cross-view evidence;
- **contradiction cases** — records of cases where two `ImageAsset` objects within one `RoomCase` yield conflicting evidence about the same claim; contradiction *preservation* (not silent resolution) is required by Bounded Scope Rev5 §8B; governed as evidence artifacts under the same rules as any other `PerceptionEvidenceArtifact`;
- **evidence fusion records** — the record of how per-image evidence was combined into one consolidated, operation-level result; operation-scoped, carrying `operationId`, `roomCaseId`, and `contributingImageAssetIds[]`;
- **per-image provenance** — the provenance chain (source class, license/eligibility record, lineage) attached to one specific `ImageAsset`/`sourceAssetId`; never aggregated away from the individual photograph (Section 3.3.0's 1:1:1 binding rule);
- **RoomCase-level provenance** — the aggregate provenance statement for a `RoomCase` as a whole (e.g. "all six ImageAssets are staged-source, same session, same lineage"); does not replace or override any individual `ImageAsset`'s own per-image provenance record.

This Decision does not itself define the annotation schema, scoring rule, or acceptance metric associated with any of the above categories — those remain governed by Evaluation Threshold and Acceptance Plan (Revision 15 pending separate acceptance of Residential-34 successor Revision 16) and its Supporting Contracts (Contract 2 Revision 10 for relation semantics in general; Contract 7, not yet drafted, for scenario/sufficiency semantics). This Decision governs only their handling, identity, and protection as data, exactly as Section 3.3's closing rule (below) already states for all perception-execution and evaluation artifacts.
...
This Decision does not define evaluation metrics, contract structure or acceptance thresholds associated with these artifacts; those remain governed by the Evaluation Threshold and Acceptance Plan (Revision 15 pending separate acceptance of Residential-34 successor Revision 16 — **[Revision 10, C-07]** citation updated from the historical "Rev13" reference, which was accurate when Revision 9 was written and is now stale) and its Supporting Contracts. This Decision governs only their handling as data.
...
Tier 1 Corpus Preparation may be considered ready for separate authorization only when this Decision is accepted; supporting contracts **1–10** under the accepted Evaluation Threshold governance route are atomically accepted and locked (Rev13 §5.2 as carried through Revision 15 and proposed Revision 16); the mandatory Diagnosability/Security sequence in the accepted Module Applicability Profile is complete; the Phase-1 Scope Decision or Execution Profile is accepted; **and** data-governance artifacts 1, 2, 3, 4, 5, 6, 7, 9 and 10 of Section 22 are each prepared, reviewed, accepted, and version-locked — not merely drafted or "ready to implement." The proposed preparation package must additionally demonstrate:
...
Acceptance of this Decision, by itself, does not pass this gate. The Contracts 1–10 dependency, mandatory Diagnosability/Security sequence, accepted Phase-1 Scope Decision or Execution Profile, and Section 22 artifact-locking dependency stated above must all be independently satisfied in the accepted order. The Phase-1 document must be accepted before final acceptance/version-locking of the pre-corpus data-governance artifacts, per Decision 18 (Section 22A describes the non-normative content of that future document only, not its sequencing).

Contract 11 (Aggregation, Uncertainty and Score-Stability Appendix) and Section 22 artifact 8 (Held-Out Sealing Procedure) are deliberately **not** included in this gate. Under the accepted ETAP sequencing, Contract 11 is prepared and locked only after development corpus/fixture creation and annotation-quality repair, using actual development denominators that do not exist before Corpus Preparation. Both are instead preconditions for held-out sealing under Section 15.1. Section 22 artifact 11 (Provider Exposure and Deletion Log schema) and the applicable clearance (Source-Generation Provider Exposure Clearance, Section 16.0.1, or Evaluation-Time Provider Exposure Clearance, Section 16.2A) are preconditions for first exposure under the relevant provider track, not for Corpus Preparation.
...
The Project Owner is asked to decide Decision 1, updated Decisions 15, 17, 18, 19, 20, and Revision 10 Decisions 38-41. Decisions 2-14 and 21-37 are accepted Revision 9 baseline decisions carried forward and not reopened.

### Decision 1 — Document acceptance

Recommended:

`ACCEPT Candidate A Test Data Handling Decision Revision 10 as the authoritative Residential-34 / RoomCase-ImageAsset test-data governance successor to accepted Revision 9.`

### Decision 2 — Source classes

Accepted Revision 9 baseline — carried forward, not reopened:
...
### Decision 15 — Corpus Preparation admission gate **[current-reference update, Revision 10]**

Recommended:

`ACCEPT Section 20, including the explicit dependencies on (a) atomic acceptance and locking of supporting contracts 1–10 under the accepted Evaluation Threshold governance route, (b) completion of the mandatory Diagnosability/Security sequence, (c) acceptance of the Phase-1 Scope Decision or Execution Profile, and (d) acceptance and version-locking of Section 22 data-governance artifacts 1, 2, 3, 4, 5, 6, 7, 9 and 10 — and the explicit exclusion of contract 11 and artifact 8, which instead gate held-out sealing under Section 15.1 — as the minimum governance evidence required before separate Tier 1 Corpus Preparation Authorization may be considered. No subset of (a)-(d) is sufficient; all are required in the accepted order.`
...
### Decision 17 — Non-authorization boundary **[current-reference update, Revision 10]**

Recommended:

`CONFIRM that acceptance does not authorize: drafting or redrafting of Contracts 1-2, drafting of Contracts 3-11, or any supporting-contract preparation without separate authorization; data-governance artifact preparation (Section 22, artifacts 1–11); corpus or fixture creation and annotation; Development/Candidate Assessment Authorization; Provider/Model Evaluation Authorization; Synthetic Source Generation Authorization; Evaluation-Time Provider Exposure Clearance; Source-Generation Provider Exposure Clearance; provider contact or exposure under either provider track (Section 16.0); repository persistence; ADR work; Phase-1 Scope Decision or Execution Profile preparation, unless separately authorized; Implementation Package; or implementation.`

### Decision 18 — Next governance step **[current-reference update, Revision 10]**

Recommended:

`CONFIRM the current sequence: Contract 1 Revision 19 and Contract 2 Revision 10 are completed and candidate-locked inputs; Contracts 3-10 remain not authorized; after completion and acceptance of the current Phase 6 synchronization, Project Context and Living Strategic Roadmap are synchronized once and the complete root package receives one final consolidated review; only after that closure may a later separately authorized Contracts 3-10 cycle proceed. After atomic Contracts 1-10 acceptance, the mandatory Diagnosability/Security sequence must be completed, then the Phase-1 Scope Decision or Execution Profile must be accepted before final acceptance/version-locking of Section 22 artifacts 1-7 and 9-10, and only then may Tier 1 Corpus Preparation Authorization be considered. This Decision authorizes none of those steps.`

### Decision 19 — Perception-execution and evaluation artifact scope **[current-reference update, Revision 10]**

Recommended:

`ACCEPT Sections 3.3, 8A and 17.1A as the governance boundary for diagnostic crops, raw provider output, VlmSceneCandidate, StructuredSceneV0 (evaluation copies), PerceptionEvidenceArtifact, the platform adapter log / provider-returned metadata / provider-account audit record / provider-internal log taxonomy (Section 3.3.5), prompt traces and evaluation reports, as required by Candidate A Bounded Scope Decision Rev3, Part M.1. This Decision does not define evaluation metrics, contract structure or acceptance thresholds for these artifacts, which remain governed by the accepted Evaluation Threshold baseline and its Phase 6 successor Revision 16.`

### Decision 20 — Operation-scoped, evaluation-run-scoped, corpus-scoped and governance/audit-scoped artifact identity **[updated, Revision 10]**

Recommended:

`ACCEPT the four artifact identity classes in Sections 3.3.1-3.3.4 and the operation-type-aware model in Section 3.3.0: valid PerceptionOperations require roomCaseId; SourceGenerationOperations do not fabricate roomCaseId before RoomCase admission; mixed-room negative fixtures use inputSetId plus atomic per-image imageAssetId/sourceAssetId identities and expected mixed_room_rejected rather than a false valid RoomCase; no identity value may be invented or used as a placeholder.`
...
### Decision 38 — Residential-34 scope reference **[new, Revision 10]**

Recommended:

`ACCEPT that Section 2, item 5 now references the complete 34-category Residential-34 scope (Bounded Scope Rev5 §6; Contract 1 Rev19 §2.3) by citation rather than restating the historical four-room list, preserving Contract 1's sole ownership of the vocabulary.`

### Decision 39 — Operation / RoomCase / ImageAsset[1-6] input model **[new, Revision 10]**

Recommended:

`ACCEPT that valid PerceptionOperations use one RoomCase and 1-6 ImageAssets, while source-generation and negative-validation fixture operations follow the separate no-fabrication identities in Section 3.3.0; same-RoomCase multi-image fusion is in scope and persistent cross-session/whole-home reasoning remains excluded.`

### Decision 40 — Operation/RoomCase/ImageAsset identity levels; sourceAssetId remains atomic **[new, Revision 10]**

Recommended:

`ACCEPT the Section 3.3.0 identity model: sourceAssetId remains atomic; every admitted photograph has one ImageAsset ↔ one imageAssetId ↔ one sourceAssetId; every RoomCase-level input and output artifact carries the complete unique contributingImageAssetIds[] set with cardinality 1..6; exact serialization and field alignment remain Contract 10 responsibilities.`

### Decision 41 — Multi-image RoomCase evidentiary categories governed as data **[new, Revision 10]**

Recommended:

`ACCEPT Section 3.3.1A's naming and data-governance treatment of same-room image sets, mixed-room negative sets, cross-view evidence, cross-view matching, duplicate-object cases, contradiction cases, evidence fusion records, per-image provenance, and RoomCase-level provenance, and CONFIRM that this Decision defines none of their annotation schemas, scoring rules, or acceptance metrics — those remain governed by the Evaluation Threshold and Acceptance Plan and its Supporting Contracts.`

---

## 25. Recommended governance outcome

Recommended outcome for this Revision 10 (this document's own status; not yet Owner-decided):

```text
STATUS (this document):
Draft — Awaiting Project Owner Review. NOT Accepted by this document.

GOVERNANCE EFFECT UPON ACCEPTANCE:
Candidate A Test Data Handling Decision Revision 10 would become
authoritative, superseding Revision 9. Until separately accepted,
Revision 9 (Accepted, Project Owner, 2026-07-16) remains authoritative,
unmodified.

STILL NOT AUTHORIZED (cumulative boundary, including Revision 10 clarifications):
...
- drafting or redrafting of Contracts 1-2, or drafting of Contracts 3-11;
- a new Candidate Lock ID for Contract 1 or Contract 2;
- preparation of the eleven data-governance artifacts (Section 22);
- repository persistence, commit, or push of this document, Contract 1 Revision 19, or Contract 2 Revision 10;
- ADR creation;
- Phase-1 Scope Decision or Execution Profile preparation, unless separately authorized;
- Implementation Package;
- implementation;
- active_locked status for any of the 34 Residential-34 categories.

ALREADY COMPLETED, IMPORTED, NOT REOPENED BY THIS DOCUMENT:
- Contract 1 Revision 19 (candidate-locked, C1-REV19-CL-001);
- Contract 2 Revision 10 (candidate-locked, C2-REV10-CL-001).
```
...
| Contracts 1-10 dependency in admission gate retained under current ETAP route | Yes — Section 20 |
...
| No stale current-revision acceptance target | Yes — Decision 1 accepts Revision 10; Revision 9 is predecessor only |
...
| Decision 18 states the current Phase 6 / one-time Context-Roadmap synchronization / final root review / Contracts 3-10 / Diagnosability-Security / Phase-1 / pre-corpus sequence | Yes — synchronized in Revision 10 |
...
| Owner Decisions | Yes |
| Residential-34 scope reference replaces historical four-room list, by citation only, not restated | Yes (new in Rev10, C-03 — Section 2 item 5, Decision 38) |
| Historical one-image-per-operation model replaced by Operation/RoomCase/ImageAsset[1-6]; multi-image exclusion narrowed to exclude only cross-session/whole-home | Yes (new in Rev10, C-01/C-02 — Section 2 items 2 and 4, Decision 39) |
| Operation-type-aware RoomCase/ImageAsset identity; sourceAssetId atomic; complete contributingImageAssetIds[] required for every RoomCase-level input/output | Yes — Sections 3.3.0-3.3.1, Decisions 20 and 40 |
| Multi-image RoomCase evidentiary categories (same-room sets, mixed-room negatives, cross-view evidence/matching, duplicate-object, contradiction, evidence fusion, per-image/RoomCase-level provenance) named and governed as data | Yes (new in Rev10, C-06 — Section 3.3.1A, Decision 41) |
| Text-only generation and mixed-room negative fixtures avoid fabricated roomCaseId | Yes — Sections 3.3.0-3.3.1A, Decision 20 |
| Every RoomCase-level request and consolidated output carries contributingImageAssetIds[] cardinality 1..6 | Yes — Sections 3.3.0-3.3.1, Decision 40 |
| Stale forward-reference to Evaluation Threshold Plan Rev13 corrected to current Rev15/proposed Rev16 | Yes (Rev10, C-07 — Section 3.3.7, found during this document's own adjacent-field self-check) |
| Sections 4-19 and 21-23 substantively retained; Section 20 sequencing synchronization, Section 24 Decision updates, and Sections 25-26/footer changes are explicitly itemized in C-09-C-14 | Yes — accurately recorded in Revision 10 Change Summary |
| Operative-text search for stale four/five-room and one-perspective-image scope language | Yes — remaining occurrences are explicitly historical quotes or Change Summary descriptions, not operative requirements |
| No repository modification or authority over predecessor/companion documents | Yes — accepted predecessors remain unchanged; ETAP Rev16 and MAP Rev19 are separate companion drafts corrected in the same review package, not by this Decision |
| Document not declared Accepted; status remains Draft — Awaiting Project Owner Review | Yes |
| SHA-256 of uploaded Contract 1 Rev19 and Contract 2 Rev10 artifacts independently verified against Candidate Lock records before use | Yes (d899a13e...329 and 758bf9b9...177, both matched exactly) |
...
Revision 10 does not supersede Revision 9 until separately accepted.
Revision 9 remains on disk, unmodified, as the current authoritative
baseline.

Residential-34 / RoomCase / ImageAsset[1-6] model:
Reflected in Sections 2, 3.3.0, 3.3.1, 3.3.1A of this document.
Corpus creation, annotation, provider/model evaluation, and
implementation remain not authorized (Section 25).

Accepted Revision 9, Bounded Scope Decision Revision 5, Contract 1
Revision 19, Contract 2 Revision 10, ETAP Revision 15 and MAP Revision 18
are not modified or reopened by this Decision. ETAP Revision 16 and MAP
Revision 19 are separate Phase 6 companion drafts and are not accepted or
controlled by this Decision.

Schema, corpus, provider, and implementation authorization:
Not granted by this document.

Repository changes performed during preparation of this Revision 10:
NO.

Repository persistence of this Revision 10:
NOT PERFORMED. Not authorized by this document.

File state (this Revision 10):
